// src/lib/CircularBuffer.js
var MuseCircularBuffer = class {
  /**
...
// src/lib/MuseDevice.js
var MuseBase = class _MuseBase {
  // Legacy Muse service UUID (Muse 2016, Muse 2)
  #LEGACY_SERVICE = 65165;
...
`);
  ...
  #createMockEEGData(value) {
    const unsigned12bit = Math.max(
      0,
      Math.min(4095, Math.round(value / 0.48828125 + 2048))
    );
    ...
    for (let i = 0; i < 12; i++) {
      ...
      if (i % 2 === 0) {
        uint8[byteOffset] = unsigned12bit >> 4 & 255;
        uint8[byteOffset + 1] = (unsigned12bit & 15) << 4 | unsigned12bit >> 8 & 15;
      } else {
        uint8[byteOffset] = uint8[byteOffset] & 240 | unsigned12bit >> 8 & 15;
        uint8[byteOffset + 1] = unsigned12bit & 255;
      }
    }
    return view;
  }
  ...
  async connect() {
    ...
    if (!this.#controlChar && !this.#isAthena) {
      ...
      if (!hasLegacyEEG) {
        console.warn(
          "[web-muse] No Interaxon EEG characteristics found on any service. 221e Muse v3 CMD/DATA characteristics stream IMU data (TLV binary), not EEG. This device may not support EEG streaming via Web Bluetooth."
        );
      }
    }
    if (this.#controlChar) {
...
var Muse = class extends MuseBase {
  constructor(options = {}) {
    super(options);
  ...
  athenaEegData(charIndex, event) {
    ...
    if (this._athenaLogCount < 3) {
      console.log(
        `[web-muse] Athena char ${charIndex}: ${bytes.length} bytes, first 30:`,
  ...
  #parseAthenaEegSubpacket(tag, dataBytes) {
    ...
    const nChannels = tag === 17 ? 4 : 8;
    const nSamples = tag === 17 ? 4 : 2;
  ...
  _parseAthenaBatterySubpacket(tag, dataBytes) {
    ...
    if (tag === 152 && dataBytes.length >= 4) {
      ...
      if (!this._batteryLogged) {
        ...
        console.log(
          `[web-muse] Athena battery (0x98): ${this.batteryLevel}%`,
          `raw first 10 bytes:`,
          Array.from(dataBytes.slice(0, 10)).map((b) => b.toString(16).padStart(2, "0")).join(" ")
        );
      }
    } else if (tag === 136 && dataBytes.length >= 4) {
      const level = dv.getUint16(2) / 512;
      if (level >= 0 && level <= 100) {
        this.batteryLevel = Math.round(level);
      }
      if (!this._batteryLogged) {
        ...
        if (this.batteryLevel === null || this.batteryLevel === 0) {
          console.log(
            `[web-muse] Battery parse may need adjustment. uint16 candidates:`,
            `@0=${dv.getUint16(0)}`,
            `@2=${dv.getUint16(2)}`,
            `@4=${dv.getUint16(4)}`,
            `@6=${dv.getUint16(6)}`,
  ...
  #decode14BitData(bytes) {
    ...
    for (let i = 0; i + 6 < bytes.length; i += 7) {
      values.push((bytes[i] | (bytes[i + 1] & 63) << 8) & 16383);
      values.push((bytes[i + 1] >> 6 | bytes[i + 2] << 2 | (bytes[i + 3] & 15) << 10) & 16383);
      values.push((bytes[i + 3] >> 4 | bytes[i + 4] << 4 | (bytes[i + 5] & 3) << 12) & 16383);
      values.push((bytes[i + 5] >> 2 | bytes[i + 6] << 6) & 16383);
    }
    return values;
  }
